...
interface LocationInfoWindowProps {
  ...
  initializeStreetView: (
    ...
    streetViewDivRef: React.RefObject<HTMLDivElement>
  ) => void;
...
const LocationInfoWindow: React.FC<LocationInfoWindowProps> = ({
  ...
  useEffect(() => {
    ...
    console.log(
      "LocationInfoWindow - Selected location:",
  ...
  const handleGetDirections = async () => {
    const { name, address, city, position } = selectedLocation;
    ...
    const query = name
      ? `${name}, ${address || city || `${position.lat},${position.lng}`}`
      : address || city || `${position.lat},${position.lng}`;

    const encodedQuery = encodeURIComponent(query);

    // Use directions instead of just search
    const url = `https://www.google.com/maps/dir/?api=1&destination=${encodedQuery}${
      origin ? `&origin=${data.location}` : ""
    }`;
    window.open(url, "_blank");
  };
    ...
    >
      <div className="p-1 font-sans bg-[#1A1F2C] text-white rounded-md w-full max-w-[375px] border border-white/10">
        ...
        <div className="flex items-center mb-1 justify-between">
          <div className="flex items-center">
            <div
              ...
              style={{
                backgroundColor:
                  selectedLocation.type === "range"
                    ? "#ff4d4d"
                    : selectedLocation.type === "shop"
                    ? "#4d94ff"
                    : selectedLocation.type === "gunsmith"
                    ? "#ffa64d"
                    : selectedLocation.type === "training"
                    ? "#4dff88"
                    : selectedLocation.type === "museum"
                    ? "#cb4dff"
                    : selectedLocation.type === "clay"
                    ? "#ffcc4d"
                    : "#ffffff",
              }}
            ></div>
            <span className="text-[10px] text-white/80 capitalize">{selectedLocation.type}</span>
          </div>
        ...
        {!isMobile && streetViewAvailable && (
          <div className="text-[8px] text-white/50 mt-0.5 text-center">Drag to look around</div>
        )}
      </div>
    </InfoWindow>
  );
