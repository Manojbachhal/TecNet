...
const CreateListingDialog = ({
      ...
      if (!editItem) {
        ...
        if (existing) {
          toast({
            ...
            description: "You already have an active listing for this firearm.",
            variant: "destructive",
          });
          onClose();
          return;
      ...
      if (success) {
        ...
        toast({
          ...
          description: editItem
            ...
            : "Your item has been listed for trade.",
        });
      }
    } catch (error: any) {
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { data, error } = await supabase.storage.from("firearm-images").upload(filePath, file, {
        upsert: true,
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from("firearm-images").getPublicUrl(filePath);
